...
export type ClubActivityType_Legacy =
  | "rating"
  | "nomination"
  | "discussion"
  | "event"
...
// Legacy ClubActivityItem interface for backward compatibility
export interface ClubActivityItem {
  id: string;
  type: ClubActivityType_Legacy;
  timestamp: string;
  user: {
    id: string;
    display_name: string | null;
...
// Legacy GroupedActivity type
export interface LegacyGroupedActivity {
  id: string;
  type: ClubActivityType_Legacy;
  timestamp: string;
  groupKey: string;
...
/**
 * Legacy function - converts new format to old format for backward compatibility
 *
 * @deprecated This function has no consumers as of 2026-01-08 and can be safely removed.
 * Kept temporarily in case any code paths were missed during the activity feed refactor.
 * Safe to remove after confirming no runtime errors in production for 1-2 weeks.
 */
export function convertToLegacyFormat(activities: GroupedActivity[]): LegacyGroupedActivity[] {
  return activities.map((a) => {
    const type: ClubActivityType_Legacy = a.category === "club" ? "general" : "general";
    ...
    return {
      ...
      action: a.action,
      details: a.items[0]?.details || undefined,
    };
  });
}
