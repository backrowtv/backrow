...
import { User, ArrowClockwise, House, Warning } from "@phosphor-icons/react";
import Link from "next/link";

const ERROR_MESSAGES = [
  { title: "Person not found!", subtitle: "This profile might have left the set." },
  { title: "Credits missing!", subtitle: "We couldn't locate this person's data." },
  { title: "Take two!", subtitle: "Something went wrong loading this profile." },
  { title: "Not in the cast!", subtitle: "This person page isn't available right now." },
];
      ...
      <div className="max-w-md w-full text-center space-y-6">
        <div className="relative mx-auto w-24 h-24">
          ...
          <div className="relative flex items-center justify-center w-24 h-24 bg-[var(--surface-2)] rounded-full border border-[var(--border)]">
            <User className="w-12 h-12 text-[var(--primary)]" weight="duotone" />
          </div>
        </div>
        ...
        <p className="text-xs text-[var(--text-muted)]">
          If this person should exist, try searching for them again.
        </p>
      </div>
    </div>
  );
