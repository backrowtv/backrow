...
export async function addTagToThread(
  ...
  try {
    ...
    const isAdmin = membership.role === 'admin' || membership.role === 'producer' || membership.role === 'director'
    ...
    const { data: newTag, error } = await supabase
      ...
      .insert({
        ...
        person_tmdb_id: ['actor', 'director', 'composer'].includes(tag.tag_type) ? tag.person_tmdb_id : null,
