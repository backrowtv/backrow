...
export function Logo({ variant = 'full', size = 'md', className = '', style }: LogoProps) {
  ...
  if (variant === 'icon') {
    return (
      <div className={`inline-flex items-center text-foreground ${className}`} style={style}>
        {theaterIconSvg}
      </div>
    )
  }
  ...
  if (variant === 'text') {
    return (
      <span 
        className={`${textSize} text-[var(--primary)] ${className}`} 
  ...
  return (
    <div className={`inline-flex items-center gap-2 ${className}`} style={style}>
      ...
      <span 
        className={`${textSize} text-[var(--primary)]`}
