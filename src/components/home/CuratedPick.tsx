...
import { getCurrentCuratedPick } from "@/app/actions/marketing";
import { getTMDBBlurDataURL } from "@/lib/utils/blur-placeholder";

export async function CuratedPick() {
  const pick = await getCurrentCuratedPick();
  ...
  return (
    <Card className="p-0 overflow-hidden relative group h-[400px] flex flex-col border-0">
      ...
      <div className="relative z-10 p-6 flex flex-col h-full justify-between text-white">
        ...
        <div>
          {pick.curator_note && (
            <p className="mb-4 line-clamp-2 text-sm opacity-90">{pick.curator_note}</p>
          )}
