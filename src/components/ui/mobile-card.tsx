    ...
    return (
      <div
        ...
        className={cn(
          ...
          variant === "outlined" && ["bg-transparent", "border border-[var(--border)]"],
          variant === "elevated" && ["bg-[var(--surface-1)]", "shadow-lg shadow-black/10"],

          // Pressable styles
          pressable && ["cursor-pointer", "active:scale-[0.98]", "active:opacity-90"],
...
const MobileCardFooter = React.forwardRef<HTMLDivElement, MobileCardFooterProps>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("p-5 pt-0", "border-t border-[var(--border)]", className)}
      {...props}
    />
  )
);
